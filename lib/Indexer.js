...
class Indexer {
  ...
  async indexObject({item, crate, crateId, id, root, memberOf, collectionStack}) {
    ...
    if (!validLicense) {
      ...
      const journeyGeo = [];
      const dateGeo = [];
      const place = crate.getProperty(item, 'place');
      const spatialCoverage = crate.getProperty(item, 'spatialCoverage');
      const journey = crate.getProperty(item, 'journey');
      const dateCreated = crate.getProperty(item, 'dateCreated');
      var isFristLocation  = true;
      if( place ) {
        for (let p in place) {
          const mloc_ref = crate.getProperty(place[p], "geo");
          if( mloc_ref && mloc_ref.length > 0 ) {
            geo.push(mloc_ref)
            // normalItem._geolocation = mloc_ref;
            // if( "latitude" in mloc_ref[0] ) {
            //   normalItem._geolocation = [{
            //     "latitude": mloc_ref[0]["latitude"],
            //     "longitude": mloc_ref[0]["longitude"]
            //   }]
            // }
          }
        }
      ...
      if( spatialCoverage ) {
        for (let pv in spatialCoverage) {
          const mloc_ref = crate.getProperty(spatialCoverage[pv], "geo");
          if( mloc_ref && mloc_ref.length > 0 ) {
            let mloc_ref_copy = JSON.parse(JSON.stringify(mloc_ref));
            if( isFristLocation && dateCreated) {
              dateGeo.push(mloc_ref_copy);
              dateGeo[0][0]['datestart'] = dateCreated[0];
              dateGeo[0][0]['dateend'] = dateCreated[0];
              dateGeo[0][0]['udatestart'] = Date.parse(dateCreated);
              dateGeo[0][0]['udateend'] = Date.parse(dateCreated);
      ...
      if( journey ) {
        for (let j in journey) {
          const mloc_ref = crate.getProperty(journey[j], "geo");
          if( mloc_ref && mloc_ref.length > 0 ) {
            journeyGeo.push(mloc_ref)
          }
        }
      }

      if( geo.length > 0 ) {
        normalItem._geolocation = [geo]
      }
      if( dateGeo.length > 0 ) {
        normalItem._dateGeolocation = [dateGeo]
      }
      if( journeyGeo.length > 0 ) {
        normalItem._journeyGeolocation = [journeyGeo];
      }

      try {
        this.log.info(`Indexing item ${id}`);
