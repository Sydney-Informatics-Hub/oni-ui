...
const {first, isEmpty, isEqual, toArray, isUndefined, clone} = require('lodash');
...
class Indexer {
  ...
  async getTextFile({itemId, filePath}) {
    if (isEmpty(this.token)) {
      ...
      try {
        let url = `${this.apiHost}/stream?id=${itemId}&path=${filePath}`;
        this.log.info(url);
        const response = await fetch(url, {
          method: 'post',
          headers: new Headers({
  ...
  async getOcflObject({id, getUrid}) {
    let url = `${this.apiHost}/object/meta?id=${id}`;
    if (!getUrid) {
      url += '&noUrid';
    }
    this.log.info(url);
  ...
  async indexCollection({item, crate, crateId, id, root, collectionMemberOf, collectionStack}) {
    this.log.info(`Indexing collection ${id}`);
  ...
  async indexObject({item, crate, crateId, id, root, memberOf, collectionStack}) {
    //this.log.info(`Indexing Object ${id}`);
    item._crateId = crateId; //We need this for the file downloads!
    ...
    if (!validLicense) {
      ...
      const placevenue = crate.getProperty(item, 'placevenue');
      if( place ) {
        for (let p in place) {
          const mloc_ref = crate.getProperty(place[p], "mappableLocation");
      ...
      if( placevenue ) {
        for (let pv in placevenue) {
          const mloc_ref = crate.getProperty(placevenue[pv], "mappableLocation");
          if( mloc_ref && mloc_ref.length > 0 ) {
            geo.push(mloc_ref)
            // normalItem._geolocation = mloc_ref;
            // if( "latitude" in mloc_ref[0] ) {
  ...
  async indexFile({parent, crateId, item, hasPart, crate, _memberOf, root, collectionStack}) {
    ...
    if (fileItem) {
      ...
      if (!validLicense) {
        ...
        const canIndexText = first(validLicense['allowTextIndex']);
