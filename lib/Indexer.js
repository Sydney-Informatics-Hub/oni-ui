...
class Indexer {
  ...
  async indexObject({item, crate, crateId, id, root, memberOf, collectionStack}) {
    ...
    if (!validLicense) {
      ...
      if( place ) {
        this.log.info(`Found place ${place[0]}`);
        this.log.info(JSON.stringify(place[0]));
        const mloc_ref = crate.getProperty(place[0], "mappableLocation");
        this.log.info(`mappableLocation = ${mloc_ref}`);
        if( mloc_ref ) {
          const mloc = crate.getItem(mloc_ref);
          if( mloc && mloc.length > 0 ) {
            normalItem._geolocations = [ mloc[0]["latitude"], mloc[0]["longitude"] ];
          }
        }
      }
